...
const PostEmployee = ()=>{
  ...
  const DeleteEmp= async (username) => {
    try {
        const delete_emp = await fetch(`http://13.212.206.255:5000/employee/${username}`,{
            method : "DELETE"
        });
  ...
  const AddEmployee = async()=> {
    try {
      ...
      const query = fetch('http://13.212.206.255:5000/employee',{
        method : 'POST',
  ...
  const GetEmployees = async()=> {
    try {
      const query = await fetch('http://13.212.206.255:5000/employee');
